...
export async function POST(req: Request) {
  let body: { mint?: unknown; reports?: unknown; rounds?: unknown; chain?: unknown };
  try {
    body = await req.json();
  } catch {
    return Response.json({ error: "Invalid request body" }, { status: 400 });
  }
  const { mint, reports, rounds, chain } = body;
  ...
  if (typeof chain !== "string" || !verifyChain(chain, "l1", mint, reports)) {
