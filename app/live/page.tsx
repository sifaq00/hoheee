...
function LiveJob() {
  ...
  useEffect(() => {
    ...
    const onMessage = (msg: Ably.Message) => {
      if (stopped) return;
      if (msg.name === "job_status") {
        const data = msg.data as { status?: string } | null;
        finish(data?.status === "failed" ? "failed" : "done");
        return;
      }
      setError(null);
      setFeed((prev) => mergePage(prev, { events: [msg.data as unknown], cursor: prev.cursor + 1 }));
    };
    void (async () => {
      try {
        client = new Ably.Realtime({ authUrl: `/api/jobs/${runId}/token` });
        channel = client.channels.get(`run:${runId}`);
        await channel.attach();
        // Rewind: everything published before attach, oldest first.
        let page = await channel.history({ untilAttach: true, limit: 100 });
        const items: unknown[] = [];
        for (;;) {
          items.push(...page.items.map((m) => m.data as unknown));
        ...
        setFeed({ events: chronological, cursor: chronological.length });
    ...
    <div className="min-h-full flex flex-col items-center px-4 py-10">
      <main className="w-full max-w-7xl flex flex-col gap-6">
        ...
        <header className="flex flex-col gap-2 border-b border-zinc-800 pb-4">
          ...
          <p className="text-sm text-zinc-400">Research tool, not financial advice. Streams progress in realtime.</p>
        </header>
      ...
      <footer className="mt-6 w-full max-w-2xl border-t border-zinc-800 pt-4 text-xs leading-relaxed text-zinc-500">
        <p>
          Live jobs run on GitHub Actions and stream events in realtime. Each analysis takes about 13-20 minutes. This system
