  ...
  return (
    <footer className="border-t border-white/[0.07]">
        ...
        {COLS.map((c) => (
          <nav key={c.head} aria-label={c.head}>
            ...
            <ul className="mt-3 flex flex-col gap-2">
              {c.links.map((l) => (
                <li key={l.label}>
                  <Link href={l.href} className="cursor-pointer font-mono text-[13px] text-zinc-400 transition-colors hover:text-[#22c55e]">
                    {l.label}
                  </Link>
                </li>
              ))}
      ...
      <div className="border-t border-white/[0.07]">
        <p className="mx-auto max-w-6xl px-4 py-5 font-mono text-[11px] text-zinc-600">© 2026 Aries — Solana token research squad.</p>
      </div>
    </footer>
  );
}
