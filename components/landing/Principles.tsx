...
export function SectionHead({ no, title, lede }: { no: string; title: string; lede: string }) {
  return (
    <Reveal>
      <p className="font-mono text-[10px] tracking-[0.22em] text-[#15803d] uppercase">{no}</p>
      <h2 className="font-display mt-3 text-3xl font-medium tracking-tight text-black sm:text-4xl">{title}</h2>
      <p className="mt-3 max-w-xl font-mono text-sm leading-relaxed text-zinc-600">{lede}</p>
    </Reveal>
  );
}
...
export default function Principles() {
  return (
    <section id="how" className="scroll-mt-24 border-t border-black/[0.07]" aria-labelledby="how-title">
