...
export default function ReportAnatomy() {
  return (
    <section id="report" className="scroll-mt-24 border-t border-black/[0.07]" aria-labelledby="report-title">
          ...
          {CARDS.map((c, i) => (
            <Reveal key={c.title} delay={(i % 2) * 90} className="h-full">
              <article className={`flex h-full flex-col gap-3 rounded-md border border-black/10 bg-white p-7 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_16px_40px_rgba(0,0,0,0.1)] ${c.live ? "border-[#22c55e] bg-[#22c55e]" : ""}`}>
                ...
                <Link href="/analyze" className={`cursor-pointer font-mono text-[13px] font-medium ${c.live ? "text-black" : "text-black"}`}>
                  Try it <span className="text-[#15803d]">→</span>
                </Link>
              </article>
            </Reveal>
          ))}
